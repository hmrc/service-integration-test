  ...
  def start(testId: TestId, externalServices: Seq[String], timeout: Duration = 60.seconds): Map[String, Int] = {
    ...
    if (externalServices.isEmpty)
      ...
      val f = extendedTimeoutClient.url(serviceManagerStartUrl)
        ...
        .post(Json.toJson(ServiceManagementStartRequest(testId.toString, externalServices.map(s => ExternalService(s)))))
...
class JsException(method: String, url: String, body: String, clazz: Class[_], errors: Seq[(JsPath, Seq[ValidationError])]) extends Exception {
